...
def get_episodes(env_name,
    ...
    model_base_filedir = os.path.join('data', env_name)
    if not os.path.exists(model_base_filedir):
        os.makedirs(model_base_filedir)
    train_file = os.path.join(model_base_filedir,'episodes_'+train_mode+str(seed)+'.data')
    
    if os.path.exists(train_file):
        dict = torch.load(train_file)
        episodes, episode_labels = dict['episodes'], dict['episode_labels']
    elif collect_mode == "random_agent":
    ...
    # torch.save({'episodes':episodes,'episode_labels':episode_labels}, train_file)
