...
class FullySupervisedLinearProbe(nn.Module):
    ...
    def forward(self, x, mask=None):
        feature_vec = self.encoder(x, mask=mask)
...
class ProbeTrainer():
    def __init__(self,
                 ...
                 epochs=100,
                 batch_size=64,
                 representation_len=256, probe_masks=False, mask_ratio=0.4):

        self.encoder = encoder
        self.probe_masks = probe_masks
        self.wandb = wandb
        self.device = device
    ...
    def generate_batch(self, episodes, episode_labels):
        ...
        for indices in sampler:
            ...
            xs, masks, labels = [], [], appendabledict()
            ...
            yield torch.stack(xs).float().to(self.device) / 255., labels,  torch.Tensor(np.array(masks)).float().to(self.device)

    def probe(self, batch, k, masks):
        if not self.probe_masks:
            masks = None
        probe = self.probes[k]
        probe.to(self.device)
        if self.fully_supervised:
            # if method is supervised batch is a batch of frames and probe is a full encoder + linear or nonlinear probe
            preds = probe(batch, mask=masks)
            ...
            with torch.no_grad():
                ...
                # else:
                #     batch = batch * (1-masks.unsqueeze(1))

                f = self.encoder(batch, mask=masks).detach()
            preds = probe(f)
        return preds
    ...
    def do_one_epoch(self, episodes, label_dicts):
        ...
        for step, (x, labels_batch, masks) in enumerate(data_generator):
    ...
    def do_test_epoch(self, episodes, label_dicts):
        ...
        for step, (x, labels_batch, masks) in enumerate(data_generator):
            for k, label in labels_batch.items():
                ...
                preds = self.probe(x, k, masks).detach().cpu()
